...
cron.schedule("0 0 0 * * *", async () => {
    ...
    for (let market_data of doc) {
        for (let stock_data of market_data.stocks) {
            ...
            const dailyGraphData = await dailyGraphInfo.findOne({ticker_id: stock_data.ticker})

            const dailyStockDataFromApi = await getStockDataFromApi(stock_data)
            const timeSeriesData = dailyStockDataFromApi['Time Series (Daily)']
