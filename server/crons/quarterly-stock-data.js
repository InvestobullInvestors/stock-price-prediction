...
cron.schedule("0 0 0 */15 * *", async () => {
    ...
    for (let market_data of doc) {
        for (let stock_data of market_data.stocks) {
            ...
            const quarterlyStockInfoForTicker = await quarterlyStockInfo.findOne({ticker_id: stock_data.ticker})
            ...
            const quarterly_stock_details = {
                ...
                quarterly_earning_growth: Number(stockDataFromApi['QuarterlyEarningsGrowthYOY']) || null,
                quarterly_revenue_growth: Number(stockDataFromApi['QuarterlyRevenueGrowthYOY']) || null,
                beta: Number(stockDataFromApi['Beta']) || null,
                fifty_two_week_high: Number(stockDataFromApi['52WeekHigh']) || null,
                fifty_two_week_low: Number(stockDataFromApi['52WeekLow']) || null,
                dividend_payout_ratio: Number(stockDataFromApi['PayoutRatio']) || null,
                dividend_date: stockDataFromApi['DividendDate'],
                shares_outstanding: Number(stockDataFromApi['SharesOutstanding']) || null,
            ...
            if (quarterlyStockInfoForTicker) {
                await quarterlyStockInfo.updateOne({ticker_id: stock_data.ticker}, {
